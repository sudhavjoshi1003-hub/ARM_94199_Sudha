...
#include"my_led.h"
volatile int exti_flag;
//The flag must be declared as volatile because the interrupt can occur
//any time and it must be able to read it and do the operation accordingly.
//If it is not declared as volatile the CPU will store it's default value
//in it's register due to optimization and when the interrupt occur it will
//not read the flag's value but instead it will read the stored value in the
//register.
